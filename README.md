...
[![VLM](https://img.shields.io/badge/VLM-supported-brightgreen)](#supported-backends)
[![Portable](https://img.shields.io/badge/portable-one%20folder-00b894)](#portable-layout)
...
- MinerU **VLM support** through the upstream `mineru[core]` install path.
...
MinerUI Portable has three separate backend/runtime layers:

1. **MinerU backend version** — the MinerU Python package/wheel used by this portable release.
2. **MinerU feature backend** — upstream MinerU extras installed into the local environment.
3. **Torch backend** — CPU/CUDA wheel selection used by PyTorch inside the local MinerU environment.
...
### MinerU feature backend support

| Feature/backend | Status in MinerUI Portable v1.0.0 | Notes |
|---|---:|---|
| `mineru[core]` | **Supported / default** | Installs upstream `vlm`, `pipeline` and `gradio` extras. |
...
- If switching CPU/CUDA backend, delete `.venv` from the launcher UI and start again.
- If package download gets stuck or corrupted, clear `.cache\uv` and retry.
- If the WebUI does not open, check whether `127.0.0.1:7860` is already used.
...
See `NOTICE.md` and `LICENSE.md` for licensing and attribution.

## License

MinerUI Portable follows the license terms of MinerU for the included MinerU code/package. See `LICENSE.md` and `NOTICE.md`.
